...
module.exports = async function (context, req) {
  ...
  try {
    ...
    if (method === 'POST' && (action === 'reset-account' || action === 'reset')) {
      handlers = [authController.resetAccount];
    }

    // GET /api/auth/roles
    if (method === 'GET' && action === 'roles') {
      handlers = [authenticate, authController.getRoleAssignments];
    }

    // POST /api/auth/roles
    if (method === 'POST' && action === 'roles') {
      handlers = [authenticate, authController.assignRole];
    }

    // GET /api/auth/executors
    if (method === 'GET' && action === 'executors') {
      handlers = [authenticate, authController.listExecutors];
    }

    // POST /api/auth/executors
    if (method === 'POST' && action === 'executors') {
      handlers = [authenticate, authController.createExecutor];
    }

    // PUT /api/auth/executors/:username
    if (method === 'PUT' && action === 'executors' && segments[1]) {
      expressReq.params.username = segments[1];
      handlers = [authenticate, authController.updateExecutor];
    }

    // POST /api/auth/executors/:username/reset-password
    if (method === 'POST' && action === 'executors' && segments[1] && segments[2] === 'reset-password') {
      expressReq.params.username = segments[1];
      handlers = [authenticate, authController.resetExecutorPassword];
    }

    // DELETE /api/auth/executors/:username
    if (method === 'DELETE' && action === 'executors' && segments[1]) {
      expressReq.params.username = segments[1];
      handlers = [authenticate, authController.deleteExecutor];
    }
